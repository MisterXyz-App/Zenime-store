/**
 * Zenime Store — halaman "Bayar via QRIS (manual)" khusus ZCoin.
 * Sama persis alurnya dengan pembayaran-manual.js (punya Premium), bedanya
 * cuma teks status & endpoint status yang dipakai (di-set dari HTML).
 */
...
(function () {
  ...
  uploadProofBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // Endpoint upload-proof-nya SAMA dengan punya Premium -- Edge Function
      // di baliknya auto-deteksi ini klaim premium atau coin lewat claim_id.
  ...
  async function pollOnce() {
    ...
    try {
      ...
      if (status === 'paid' || status === 'berhasil') {
        setPillState('ok', 'ZCoin ditambahkan');
